 ...
 */
int han_write_num(int negative, int ind, char buffer[],
	int flags, int width, int precision, int size)
{
	...
	return (handle_write_num(ind, buffer, flags, width, precision,
		len, padd, extra_ch));
}

/**
 * handle_write_num - Write a number using a bufffer
 * @ind: Ind at which the number starts on  buffer
 * @buffer: Buff
 * @flags: Flags
 * @width: width
 ...
 */
int handle_write_num(int ind, char buffer[],
...
/**
 * write_unsign - outputs an unsigned number
 * @negative: Num indi if the num is negative
 * @ind: Ind at which the number starts in the buffer
